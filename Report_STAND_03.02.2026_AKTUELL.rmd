...
- **Dataset A (Company Master)**: Contains company-level metadata for all SPI constituents including geographic information. This dataset serves as the backbone for all joins and analyses.

- **Dataset B (Price Data)**: Daily stock price data downloaded from Yahoo Finance, used for computing returns, volatility, and other financial metrics.
...
Dataset A contains comprehensive company-level metadata for all 188 SPI (Swiss Performance Index) constituents. This dataset includes:

- **Company identifiers**: Ticker symbols, company names, ISIN codes
- **Classification data**: Sector and size bucket assignments
- **Geographic information**: Address, canton, coordinates (latitude/longitude), zip code, city, district
...
PARAMS <- list(
  # Master data file (semicolon-delimited CSV)
  master_file = "SPI_tickers_fixed_excel.csv",
...
load_master <- function(filepath) {
  ...
  # Read semicolon-delimited CSV with UTF-8 encoding
  raw <- read_delim(
    filepath,
    delim = ";",
...
# Panel D: Cumulative distribution function (CDF)
p_ecdf <- ggplot(returns_viz_data, 
                 aes(x = ret_monthly, color = size_bucket)) +
  stat_ecdf(geom = "step", linewidth = 1.2, alpha = 0.8) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50", alpha = 0.5) +
  scale_color_manual(values = c("Large" = "#d7191c", "Mid" = "#fdae61", "Small" = "#2c7bb6")) +
  scale_x_continuous(labels = scales::percent_format(), limits = c(-0.3, 0.3)) +
  scale_y_continuous(labels = scales::percent_format()) +
  theme_minimal(base_size = 11) +
  labs(title = "D) Empirical Cumulative Distribution (ECDF)",
       subtitle = "Probability of return ≤ x-axis value",
       x = "Monthly Return", y = "Cumulative Probability",
       color = "Size Bucket") +
  theme(plot.title = element_text(face = "bold"),
        legend.position = "bottom")

# Combine panels
(p_boxplot + p_violin) / (p_ridge + p_ecdf) +
  plot_annotation(
...
- **ECDF**: Shows probability of achieving specific return thresholds. Steeper curves = more concentrated around mean.
...
# Panel B: Raincloud plot (half-violin + box + jitter)
library(ggdist)
p_raincloud <- monthly_returns %>%
  filter(size_bucket %in% c("Small", "Large"), !is.na(ret_monthly)) %>%
  ggplot(aes(x = size_bucket, y = ret_monthly, fill = size_bucket)) +
  # Half violin (distribution)
  stat_halfeye(adjust = 0.5, width = 0.6, .width = 0, point_colour = NA, alpha = 0.7) +
  # Box plot
  geom_boxplot(width = 0.15, outlier.shape = NA, alpha = 0.5) +
  # Individual points (jittered)
  geom_point(alpha = 0.1, size = 1, position = position_jitter(width = 0.05, seed = 42)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  scale_fill_manual(values = c("Large" = "#d7191c", "Small" = "#2c7bb6")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(-0.3, 0.3)) +
  theme_minimal(base_size = 12) +
  labs(title = "B) Raincloud Plot: Full Distribution Comparison",
       subtitle = "Combines density (half-violin), quartiles (box), and raw data (points)",
       x = "Size Bucket", y = "Monthly Return") +
  theme(plot.title = element_text(face = "bold"),
        legend.position = "none",
        panel.grid.major.x = element_blank())
...
# Panel A: Box plot + violin (Zug vs Other)
p_zug_box <- ggplot(zug_comparison, 
                   aes(x = canton_group, y = ret_monthly, fill = canton_group)) +
  ...
  labs(title = "A) Return Distribution: Zug vs Other Cantons",
       subtitle = "Violin (density) + Box (quartiles)",
