...
knitr::opts_chunk$set(
  echo = FALSE,          # Hide code by default in knitted output
...
# Keep data-loading chunk silent in knitted output.
# The key facts are communicated in the narrative text below.
invisible(company_master)
```
...
```{r dataset-a-variables, echo=FALSE}
...
```{r dataset-a-quality, echo=FALSE, fig.width=14, fig.height=8, fig.cap="Figure 1: Data Quality Assessment for Dataset A"}
...
# Build an informative size table with shares and a diversification benchmark
size_dist <- company_master %>%
  count(Size, name = "Count") %>%
  mutate(
    share_pct = (Count / sum(Count)) * 100,
    Size = factor(Size, levels = c("Large", "Mid", "Small"))
  )

p2 <- ggplot(size_dist, aes(x = Size, y = share_pct, fill = Size)) +
  geom_col(alpha = 0.85, width = 0.7) +
  geom_hline(yintercept = 33.3, linetype = "dashed", color = "gray50") +
  geom_text(
    aes(label = paste0(Count, " firms (", round(share_pct, 1), "%)")),
    vjust = -0.35,
    size = 3.6
  ) +
  scale_fill_manual(values = c("Large" = "#d7191c", "Mid" = "#fdae61", "Small" = "#2c7bb6")) +
  scale_y_continuous(labels = scales::label_percent(scale = 1), limits = c(0, 95)) +
  theme_minimal(base_size = 11) +
  labs(
    title = "B) Size Mix: Share of Index Constituents",
    subtitle = "Dashed line = equal-share benchmark (33.3%)",
    x = "Size bucket",
...
- **Size verdict for analysis design**: The size mix is highly imbalanced (small-cap dominant). This means any unconditional “size premium” can be driven by composition and sample-volume effects, so we treat hypothesis tests and confidence intervals as mandatory before drawing investment conclusions.
...
```{r setup-ds-B, include=FALSE, message=FALSE, warning=FALSE}
...
```{r feature-engineering, include=FALSE, results="hide"}
# ============================================================================
...
```{r data-validation, echo=FALSE, results="hide", fig.width=14, fig.height=6, fig.cap="Figure 2: Data Validation - Geographic Coverage"}
# ============================================================================
...
```{r descriptive-table-1, echo=FALSE}
# ============================================================================
...
```{r descriptive-table-2, echo=FALSE, warning=FALSE}
# ============================================================================
...
```{r pareto-1, echo=FALSE, warning=FALSE}
# ============================================================================
# PARETO CHART
# ============================================================================
...
```{r descriptive-table-3, echo=FALSE}
# ============================================================================
...
```{r concentration-analysis, echo=FALSE, results="hide", fig.width=14, fig.height=8, fig.cap="Figure 4: Concentration Risk Assessment"}
# ============================================================================
...
```{r load-returns-data, echo=FALSE, eval=FALSE}
# ============================================================================
...
```{r returns-descriptive-stats, echo=FALSE, eval=FALSE}
# ============================================================================
...
```{r returns-distributions, echo=FALSE, eval=FALSE, fig.width=16, fig.height=10, fig.cap="Figure 5: Return Distribution Comparison by Size Bucket"}
# ============================================================================
...
```{r performance-dynamics, echo=FALSE, eval=FALSE, fig.width=14, fig.height=10, fig.cap="Figure 6: Performance Dynamics by Size Bucket"}
# ============================================================================
...
```{r area-chart-cumulative, echo=FALSE, eval=FALSE, fig.width=12, fig.height=6, fig.cap="Figure: Cumulative Returns by Size Bucket (Area Chart)"}
# ============================================================================
...
```{r prepare-map1-data, include=FALSE, message=FALSE, warning=FALSE, results="hide"}
# ============================================================================
...
```{r prepare-map2-data, include=FALSE, results="hide"}
# ============================================================================
...
```{r heatmap-canton-sector, echo=FALSE, eval=FALSE, fig.width=14, fig.height=10, fig.cap="Figure: Canton x Sector Mean Returns Heatmap"}
# ============================================================================
...
In this chapter, we test each client hypothesis with one clear statistical step and one supporting visualization. We use Wilcoxon tests for group comparisons (robust to non-normal returns) and a linear regression model to control for sector and size composition when evaluating location effects.

## Hypothesis 1: Small-Cap vs Large-Cap Returns

We test whether small-cap firms outperform large-cap firms in monthly returns:

- **H~0~**: The return distributions of Small and Large firms are equal.

- **H~A~**: Small-cap monthly returns are higher than large-cap monthly returns.

We report p-value, effect size (Cohen's d), and a practical interpretation for investors.
...
```{r wilcoxon-test-size-premium, echo=FALSE, eval=FALSE}
# ============================================================================
...
- If the p-value is below 0.05, we reject H0 and conclude that small-cap returns are statistically higher in this sample.
- Cohen's d tells us whether the difference is economically meaningful, not only statistically detectable.
- For portfolio decisions, the key question is whether the return advantage remains attractive after accounting for volatility and drawdowns.

### Visualization: Mean Comparison with Confidence Intervals

```{r wilcoxon-test-1-viz, echo=FALSE, eval=FALSE, fig.width=12, fig.height=6, fig.cap="Figure 9: Size Premium Hypothesis Test - Distribution and uncertainty"}
# ============================================================================
# VISUALIZATION FOR HYPOTHESIS 1
# ============================================================================

# Prepare data for clear group comparison
...
# Compute sample size and central tendency for labels
size_summary <- size_plot_data %>%
  group_by(size_bucket) %>%
  summarise(
    n_obs = n(),
    median_return = median(ret_monthly, na.rm = TRUE),
...
# Informative boxplot with overlaid jittered observations
p_size_dist <- ggplot(size_plot_data, aes(x = size_bucket, y = ret_monthly, fill = size_bucket)) +
  geom_boxplot(width = 0.45, outlier.shape = NA, alpha = 0.65) +
  geom_jitter(width = 0.16, alpha = 0.12, size = 1.1, color = "gray30") +
  geom_point(
    data = size_summary,
    aes(x = size_bucket, y = median_return),
    inherit.aes = FALSE,
    size = 3,
    shape = 23,
    fill = "white",
  ...
  labs(
    title = "Small vs Large: full return distributions, not only means",
    subtitle = "White diamonds mark group medians; points show monthly observations",
    x = NULL,
    y = "Monthly return"
  ) +
  theme_minimal(base_size = 12) +
...
# Compact CI panel for direct uncertainty comparison
size_ci <- size_plot_data %>%
  group_by(size_bucket) %>%
  summarise(
    mean_return = mean(ret_monthly, na.rm = TRUE),
    se = sd(ret_monthly, na.rm = TRUE) / sqrt(n()),
...
p_size_ci <- ggplot(size_ci, aes(x = size_bucket, y = mean_return, color = size_bucket)) +
  geom_point(size = 4) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.12, linewidth = 1) +
  geom_text(aes(label = paste0("n = ", scales::comma(n_obs))), nudge_y = 0.004, size = 3.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  scale_color_manual(values = c("Large" = "#377eb8", "Small" = "#e41a1c")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(
    title = "Mean returns with 95% confidence intervals",
    subtitle = "Intervals reflect estimation uncertainty in each group",
    x = NULL,
...
# Combine into one hypothesis figure
p_size_dist + p_size_ci
```


## Hypothesis 2: Does Zug Outperform Other Cantons?
...
```{r wilcoxon-test-zug-premium, echo=FALSE, eval=FALSE}
# ============================================================================
...
- A low p-value supports an unconditional Zug premium, but this is not yet a causal location effect.
- Because Zug has relatively few firms, estimates are less stable and should be read with caution.
- The decisive check is the regression with sector and size controls in the next section. If the Zug coefficient weakens, the premium is likely composition-driven.

### Visualization: Zug vs Other Cantons Comparison

```{r wilcoxon-test-2-viz, echo=FALSE, eval=FALSE, fig.width=14, fig.height=6, fig.cap="Figure 10: Zug Advantage Hypothesis Test - Distribution and canton benchmark"}
# ============================================================================
# VISUALIZATION FOR HYPOTHESIS 2
# ============================================================================

# Distribution plot: better for comparing shape and median than histogram facets
zug_labels <- zug_comparison %>%
  group_by(canton_group) %>%
  summarise(n_obs = n(), .groups = "drop") %>%
  mutate(label = paste0(canton_group, "\n(n = ", scales::comma(n_obs), ")"))

p_zug_dist <- ggplot(zug_comparison, aes(x = canton_group, y = ret_monthly, fill = canton_group)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  scale_x_discrete(labels = setNames(zug_labels$label, zug_labels$canton_group)) +
  scale_fill_manual(values = c("Zug" = "#e41a1c", "Other Cantons" = "#377eb8")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(
    title = "Zug vs other cantons: distribution comparison",
    subtitle = "Boxplots and medians make central tendency and spread easy to compare",
    x = NULL,
    y = "Monthly return"
  ) +
  theme_minimal(base_size = 12) +
...
# Benchmark panel: rank cantons and highlight Zug clearly
canton_means <- monthly_returns %>%
  ...
  summarise(
    mean_return = mean(ret_monthly, na.rm = TRUE),
    n_obs = n(),
    se = sd(ret_monthly, na.rm = TRUE) / sqrt(n_obs),
    .groups = "drop"
  ) %>%
  mutate(is_zug = canton == "Zug") %>%
  arrange(desc(mean_return))

p_zug_rank <- ggplot(canton_means, aes(x = reorder(canton, mean_return), y = mean_return, color = is_zug)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_return - 1.96 * se, ymax = mean_return + 1.96 * se), width = 0.15, alpha = 0.7) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  coord_flip() +
  scale_color_manual(values = c("FALSE" = "gray45", "TRUE" = "#e41a1c"), labels = c("FALSE" = "Other", "TRUE" = "Zug")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.05)) +
  labs(
    title = "Canton benchmark with 95% confidence intervals",
    subtitle = "Zug highlighted in red for direct ranking context",
    x = NULL,
    y = "Mean monthly return",
    color = NULL
  ) +
...
p_zug_dist + p_zug_rank +
  plot_annotation(
    title = "Hypothesis 2: Is Zug really different?",
    subtitle = sprintf("Wilcoxon one-sided p-value = %.4f", w_test_zug$p.value),
...
```{r linear-regression, echo=FALSE, eval=FALSE}
# ============================================================================
# LINEAR REGRESSION: RETURN DRIVERS
# ============================================================================
...
```{r regression-diagnostics, echo=FALSE, eval=FALSE, fig.width=14, fig.height=10, fig.cap="Figure 11: Linear Regression Diagnostics"}
# ============================================================================
...
```{r model-limitations, echo=FALSE, eval=FALSE}
...
```{r investment-recommendations, echo=FALSE, eval=FALSE}
...
```{r final-risk-map, echo=FALSE, eval=FALSE, fig.width=14, fig.height=6, fig.cap="Figure 12: Final Risk Assessment - Volatility and Concentration"}
# ============================================================================
