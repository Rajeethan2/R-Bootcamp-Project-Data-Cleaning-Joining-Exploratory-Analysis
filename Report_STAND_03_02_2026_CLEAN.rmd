...
We cross-checked the assignment brief against the current report structure.

| Requirement from assignment brief | Status | Current report status |
|---|---|---|
| Use at least two datasets and merge them | ✅ | Company metadata is joined with Yahoo price history. |
| Include numeric + categorical + date or geographic variables | ✅ | Returns, sectors/sizes/cantons, timestamps, and coordinates are all used. |
| Explain data cleaning and preparation in text | ⚠️ | Expanded narrative and chunk comments to make cleaning steps easier to follow. |
| Include strong graphical analysis | ⚠️ | Reworked hypothesis plots to prioritize comparison clarity and sample size visibility. |
| Include at least one dedicated statistical model/test | ✅ | Wilcoxon tests and linear regression are explicitly reported and interpreted. |
| Deliver a client-facing storyline with conclusions and limitations | ⚠️ | Tightened transition from assumptions to results to recommendations. |
| Keep report concise and readable | ⚠️ | Reduced repeated interpretation text and consolidated hypothesis reporting. |
...
library(ggdist)         # Distribution geoms for uncertainty plots
...
This section directly tests each client hypothesis with one primary test, an
effect size, and an interpretable conclusion. We keep the methods simple and
transparent for a non-technical audience.

## Hypothesis 1: Size Premium in Risk-Adjusted Performance

**Hypothesis statement**

- **H0**: Small-cap and large-cap firms have the same return distribution.
- **H1**: Small-cap firms have higher monthly returns than large-cap firms.

**Why this test?** We use a one-sided Wilcoxon rank-sum test because return
series are usually skewed and heavy-tailed. We also report a rank-biserial
correlation (RBC) effect size and a bootstrap confidence interval for the
median return difference.

```{r hypothesis-1-test, echo=TRUE, eval=FALSE}
# ============================================================================
# HYPOTHESIS 1: SMALL-CAP VS LARGE-CAP
# ============================================================================

# Pull monthly return vectors by size bucket
...
# Main inferential test (one-sided): Small > Large
w_test_size <- wilcox.test(
  x = small_returns,
  y = large_returns,
  alternative = "greater",
  paired = FALSE,
...
n_small <- length(small_returns)
n_large <- length(large_returns)
rbc_size <- (2 * as.numeric(w_test_size$statistic) / (n_small * n_large)) - 1

# Bootstrap CI for median difference (Small - Large)
set.seed(42)
boot_diff <- replicate(
  2000,
  median(sample(small_returns, replace = TRUE)) -
    median(sample(large_returns, replace = TRUE))
)
ci_median_diff_size <- quantile(boot_diff, probs = c(0.025, 0.975), na.rm = TRUE)

# Keep Cohen's d for comparability with prior report draft
cohens_d <- (mean(small_returns) - mean(large_returns)) /
  sqrt((var(small_returns) + var(large_returns)) / 2)

# Print compact decision summary
cat("H1 Wilcoxon W:", as.numeric(w_test_size$statistic), "\n")
cat("H1 p-value:", signif(w_test_size$p.value, 4), "\n")
cat("H1 RBC effect size:", round(rbc_size, 3), "\n")
cat("H1 median diff 95% CI:",
    sprintf("[%.4f, %.4f]", ci_median_diff_size[1], ci_median_diff_size[2]), "\n")
```

```{r hypothesis-1-plot, echo=TRUE, eval=FALSE, fig.width=12, fig.height=6, fig.cap="Figure: Hypothesis 1 - Small vs Large Monthly Returns (with sample sizes)"}
# Build a clean comparison plot with sample sizes and medians
size_plot_data <- monthly_returns %>%
  filter(size_bucket %in% c("Small", "Large"), !is.na(ret_monthly)) %>%
  mutate(size_bucket = factor(size_bucket, levels = c("Large", "Small")))

size_labels <- size_plot_data %>%
  group_by(size_bucket) %>%
  summarise(n = n(), .groups = "drop") %>%
  mutate(label = paste0(as.character(size_bucket), "\n(n = ", scales::comma(n), ")"))

ggplot(size_plot_data, aes(x = size_bucket, y = ret_monthly, fill = size_bucket)) +
  geom_boxplot(width = 0.55, alpha = 0.7, outlier.alpha = 0.25) +
  stat_summary(fun = median, geom = "point", size = 2.8, color = "black") +
  scale_x_discrete(labels = setNames(size_labels$label, size_labels$size_bucket)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  scale_fill_manual(values = c("Large" = "#377eb8", "Small" = "#e41a1c")) +
  labs(
    title = "Small-caps show distributional differences, not just mean differences",
    subtitle = "Median markers and sample sizes make the comparison transparent",
    x = NULL,
    y = "Monthly return"
  ) +
  theme_minimal(base_size = 12) +
...
If `p < 0.05`, we reject H0 and conclude that small-cap returns are
stochastically higher than large-cap returns in this sample. We interpret RBC
as: ~0.1 small, ~0.3 medium, ~0.5 large practical effect.

## Hypothesis 2: Zug Return Advantage

**Hypothesis statement**

- **H0**: Zug firms and non-Zug firms have the same return distribution.
- **H1**: Zug firms have higher monthly returns than non-Zug firms.

We test this in two steps: (1) an unconditional Wilcoxon test and (2) a
regression that controls for sector and size composition.

```{r hypothesis-2-test, echo=TRUE, eval=FALSE}
# ============================================================================
# HYPOTHESIS 2: ZUG VS NON-ZUG
# ============================================================================

zug_comparison <- monthly_returns %>%
  filter(!is.na(canton), !is.na(ret_monthly)) %>%
  mutate(is_zug = if_else(canton == "Zug", "Zug", "Other cantons"))
...
other_returns <- zug_comparison %>%
  filter(is_zug == "Other cantons") %>%
  pull(ret_monthly)

w_test_zug <- wilcox.test(
  x = zug_returns,
  y = other_returns,
  alternative = "greater",
  paired = FALSE
...
n_zug <- length(zug_returns)
n_other <- length(other_returns)
rbc_zug <- (2 * as.numeric(w_test_zug$statistic) / (n_zug * n_other)) - 1

set.seed(42)
boot_diff_zug <- replicate(
  2000,
  median(sample(zug_returns, replace = TRUE)) -
    median(sample(other_returns, replace = TRUE))
)
ci_median_diff_zug <- quantile(boot_diff_zug, probs = c(0.025, 0.975), na.rm = TRUE)

cat("H2 Wilcoxon W:", as.numeric(w_test_zug$statistic), "\n")
cat("H2 p-value:", signif(w_test_zug$p.value, 4), "\n")
cat("H2 RBC effect size:", round(rbc_zug, 3), "\n")
cat("H2 median diff 95% CI:",
    sprintf("[%.4f, %.4f]", ci_median_diff_zug[1], ci_median_diff_zug[2]), "\n")
```

```{r hypothesis-2-plot, echo=TRUE, eval=FALSE, fig.width=12, fig.height=6, fig.cap="Figure: Hypothesis 2 - Zug vs non-Zug return distributions"}
# Visual check for location effect with sample size labels
zug_labels <- zug_comparison %>%
  count(is_zug) %>%
  mutate(label = paste0(is_zug, "\n(n = ", scales::comma(n), ")"))

ggplot(zug_comparison, aes(x = is_zug, y = ret_monthly, fill = is_zug)) +
  geom_violin(trim = FALSE, alpha = 0.35, color = NA) +
  geom_boxplot(width = 0.18, outlier.alpha = 0.2) +
  stat_summary(fun = median, geom = "point", size = 2.8, color = "black") +
  scale_x_discrete(labels = setNames(zug_labels$label, zug_labels$is_zug)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  scale_fill_manual(values = c("Zug" = "#1b9e77", "Other cantons" = "#7570b3")) +
  labs(
    title = "Zug vs non-Zug returns: effect must be judged with uncertainty",
    subtitle = "Unconditional comparison; sector-adjusted evidence comes from regression",
    x = NULL,
    y = "Monthly return"
  ) +
  theme_minimal(base_size = 12) +
...
## Regression Check for Hypothesis 2 and Drivers of Returns

```{r regression-return-drivers, echo=TRUE, eval=FALSE}
# ============================================================================
# LINEAR MODEL: RETURN DRIVERS WITH CONTROLS
# ============================================================================

regression_data <- monthly_returns %>%
  filter(!is.na(ret_monthly), !is.na(canton), !is.na(sector), !is.na(size_bucket))

# Additive model: isolates canton effect while controlling for sector and size
model_additive <- lm(ret_monthly ~ canton + sector + size_bucket,
                     data = regression_data)

# Optional interaction model: allows canton effects to differ by sector
model_interaction <- lm(ret_monthly ~ canton * sector + size_bucket,
                        data = regression_data)

# Key outputs for reporting
model_additive_tidy <- broom::tidy(model_additive, conf.int = TRUE)
model_fit <- broom::glance(model_additive)

cat("Adjusted R-squared:", round(model_fit$adj.r.squared, 3), "\n")
cat("Model F-stat p-value:", signif(model_fit$p.value, 4), "\n")

# Example: pull Zug coefficient if available
zug_coef <- model_additive_tidy %>%
  filter(str_detect(term, "canton"), str_detect(term, "Zug"))
print(zug_coef)
```

**Interpretation template**

For H2, the key question is whether the Zug coefficient remains positive and
statistically meaningful after sector and size controls. If not, the perceived
"Zug premium" is likely composition-driven rather than location-driven.

## Hypothesis 3: Concentration Risk Across Canton-Sector Cells

**Hypothesis statement**

- **H0**: Returns are evenly distributed across canton-sector combinations.
- **H1**: Returns are concentrated in specific canton-sector combinations.

We evaluate this hypothesis with two complementary indicators: (i) HHI
concentration by canton and (ii) explanatory power of canton/sector terms in
regression.

```{r hypothesis-3-summary, echo=TRUE, eval=FALSE}
# Practical concentration checks
cat("High concentration cantons (HHI >= 2500):",
    sum(canton_hhi$concentration_level == "High (HHI ≥ 2500)", na.rm = TRUE),
    "of", nrow(canton_hhi), "\n")
cat("Additive model R-squared:", round(summary(model_additive)$r.squared, 3), "\n")
```

**Interpretation template**

If several cantons have high HHI and the canton-sector structure explains a
meaningful share of return variation, we treat concentration risk as real and
investment-relevant.

## Model Limitations

- Monthly returns are noisy; low R² is normal in cross-sectional return models.
- Omitted variables (valuation, leverage, macro factors) may bias coefficients.
- Panel dependence may understate uncertainty; clustered errors are a next step.
- Survivorship bias may inflate estimated performance.
...
cat(sprintf("  - Wilcoxon W: %.0f\n", as.numeric(w_test_size$statistic)))
cat(sprintf("  - p-value (one-sided): %.4f\n", w_test_size$p.value))
cat(sprintf("  - RBC effect size: %.3f\n", rbc_size))
cat(sprintf("  - Median diff 95%% CI: [%.4f, %.4f]\n",
            ci_median_diff_size[1], ci_median_diff_size[2]))
cat(sprintf("  - Cohen's d (legacy comparability): %.3f\n\n", cohens_d))

cat("H2: ZUG ADVANTAGE\n")
cat("-----------------\n")
cat(sprintf("Result: %s\n",
           ifelse(w_test_zug$p.value < 0.05, "SUPPORTED (unconditional)",
                  "NOT SUPPORTED (unconditional)")))
cat(sprintf("  - Wilcoxon W: %.0f\n", as.numeric(w_test_zug$statistic)))
cat(sprintf("  - p-value (one-sided): %.4f\n", w_test_zug$p.value))
cat(sprintf("  - RBC effect size: %.3f\n", rbc_zug))
cat(sprintf("  - Median diff 95%% CI: [%.4f, %.4f]\n",
            ci_median_diff_zug[1], ci_median_diff_zug[2]))
cat("  - Sector-adjusted decision: use canton terms from model_additive\n\n")

cat("H3: CONCENTRATION RISK\n")
cat("----------------------\n")
cat("Result: SUPPORTED when HHI concentration and canton-sector model structure align\n")
cat(sprintf("  - Additive model R²: %.3f (%.1f%% variance explained)\n",
...
- **H1:** We decide based on the Wilcoxon p-value, RBC effect size, and median
  difference confidence interval, not on means alone.
- **H2:** We first test Zug vs non-Zug directly, then verify whether the effect
  survives sector and size controls in regression.
- **H3:** We treat concentration as a risk-management issue, because geographic
  allocation can unintentionally become a sector bet.
